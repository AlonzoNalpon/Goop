...
<<<<<<< Updated upstream
    ...
    return false;
=======
  gObjID FontManager::LoadFont(std::string const& name, std::string const& path, GLint fontSize)
  {
    FT_Face face;
    if (FT_New_Face(library, path.c_str(), 0, &face))
    {
    ...
    if (m_fontsLT.find(name) != m_fontsLT.end())
    {
      throw GE::Debug::Exception<FontManager>(GE::Debug::LEVEL_ERROR, ErrMsg("Font of this name exists: " + name));
    }

    // Add the name to the lookup table
    m_fontsLT[name] = m_fonts.size();

    // Add this font
    m_fonts.push_back(std::map<uChar, Character>());
    FontMap& fontMap{ m_fonts.back() };
  ...
  gObjID FontManager::GetFontID(std::string const& name)
  {
    return m_fontsLT.find(name) == m_fontsLT.end()? BAD_OBJ_ID : m_fontsLT.at(name);
  }
